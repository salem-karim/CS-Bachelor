...
auto printGeneratedValues = [](const string &generatorName,
                               const auto &values) {
...
auto check_property = [](const auto &generator, const auto &property,
  ...
  printGeneratedValues(generatorName,
                       values); // should be commented out in the final version!
...
TEST_CASE("Properties") {
  ...
  check_property(generate_ints_greater_than_1, prop_0_to_any_nonzero_int_is_0,
                 "generate ints");

  cout << "Property: any int to power 0 is 1" << endl;
  check_property(generate_ints_greater_than_1, prop_anyIntToPower0Is1,
                 "generate ints");
  ...
  cout << "Property: next power of x is previous power of x multiplied by x"
       << endl;
  check_property(generate_ints_greater_than_2_less_sqrt_maxInt,
                 prop_nextPowerOfXIsPreviousPowerOfXMultipliedByX,
                 "generate greater than 2 and less than sqrt of maxInt");
  check_property(generate_ints_greater_than_sqrt_maxInt,
                 prop_nextPowerOfXIsPreviousPowerOfXMultipliedByX,
                 "generate greater than sqrt of maxInt");
}
