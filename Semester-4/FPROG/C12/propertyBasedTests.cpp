...
auto generate_ints = [](const int min, const int max){
    if(min > max) { // when lower range is larger than upper range, just return empty vector
        return vector<int>();
    }
    ...
    if(max - min <= 100){ // if there not enough int values in the range, just return it fully
...
auto check_property = [](const auto& generator, const auto& property, const string& generatorName){
    ...
    printGeneratedValues(generatorName, values); // should be commented out in the final version!
