...
void compareDice(const Dice& dice1, const Dice& dice2, const std::string& diceNames) {
    ...
    for (int value1 : dice1) {
        for (int value2 : dice2) {
            if (value1 > value2) {
                ++wins1;
                std::cout << "(" << value1 << ", " << value2 << ") - " << diceNames.substr(0, diceNames.find(" vs ")) << " wins\n";
            } else if (value1 < value2) {
                ++wins2;
                std::cout << "(" << value1 << ", " << value2 << ") - " << diceNames.substr(diceNames.find("vs ") + 3) << " wins\n";
            } else {
                std::cout << "(" << value1 << ", " << value2 << ") - Draw\n";
            }
        }
    }
