...
//to whomever reads this: this is the result of over caffeinated after-work programming
...
//resolves calculations within Parentheses and eliminates them from list once done
...
//finds highest order of operation and sends it into the calculation, then adjusts list to reflect results
...
int main(){
    ...
    while(1){
        ...
        //terrible solution to add the first Node element, loops is basically a boolean
        if(!loops){
        ...
        //turns input into node via function, then adds it at the end of the linked list
        head = addLast(head, inputToNode(input));
        printf("\nTerm: ");
        printNodes(head);
    }
    if(verifyInput(head)){
        //function in while checks for parentheses, once there are none left, while loop is exited
        while(ParentExists(head)){
            head = resolveParenthesis(head);
        }

        //the linked list shrinks thanks to the calculate function, once it has fully finished its calculations, the result is printed one last time and the loop is exited
...
//node Creations, should be self explanatory, malloc to reserve memory, then return pointer to the newly created Node
...
//goes from head to the end of the linked list, where the new Node is then added, returns head with finished modifications
struct node *addLast(struct node * head, struct node * newNode){
...
//compares addresses saved in pointer to find the previous element in the linked list
struct node *findPrevious(struct node * head, struct node * node){
    struct node * temp = head;
...
//search for a point operator from a specified node, distinction needed due to calculations within Parentheses
struct node *findFirstPointOperator(struct node * startNode){
    struct node * temp = startNode;
    //while loop goes to NULL then checks whether it is the first closing Parenthesis it can find
    while(temp->next!=NULL){
        if(temp->type == parenthesis_type && temp->operator == ')'){
            break;
        }

        //conditions check for enum operator type and only if that condition is fulfilled the operator within the node is checked for its identity
        if(temp->next->type == operator_type && (temp->next->operator == '*' || temp->next->operator == '/')){
            //returns a pointer to the element within the linked list that fulfils the functions condition (first point operator)
            return temp->next;
        }
        temp = temp->next;
    }
...
//works exactly the same as findFirstPointOperator, except it searches for '+' and '-'
struct node *findFirstDashOperator(struct node * startNode){
    struct node * temp = startNode;
...
struct node *findLastParenthesisOpen(struct node * head){
    ...
    //count how many opening parenthesis are in list, old code, replaced by ParentExists function
    /*while(temp != NULL){
        if(temp->type == parenthesis_type && temp->operator == '(')
            ++o_p_count;
    ...
    temp = head;*/
    //by decreasing the count for every parenthesis found, the node which lowers the count to 0 is the last opening parenthesis, if there are none...
    ...
    printf("\n\n\nERROR: UNEXPECTEDLY REACHED END OF 'findLastParenthesisOpen'\n\n\n");
    return temp;
}
...
struct node *resolveParenthesis(struct node * head){
    ...
    //while loop continues to calculate until there is only one number between the parenthesis
...
//returns the head in case of a change to the list, adjusts the list by removing the Parentheses around the result
struct node *eliminateParent(struct node * head, struct node * pNode){
    struct node * tempParentOpen = pNode;
    struct node * tempParentClose = pNode->next->next;

    //if the Parentheses is in First position, properly deal with the removal of the head of the linked list
    if(tempParentOpen == head){
        ...
        //Number between brackets gets whatever is after the enclosing bracket as next
...
//for loop purposes, counts the total amount of opening parentheses in linked list
int ParentExists(struct node * startNode){
    struct node * temp = startNode;
    int parentCount = 0;
    while(temp!=NULL){
...
//determines where, how and what to calculate and then sends the necessary information into the Calc function. Once the result returns, makes the necessary modifications and removals...
...
void calculate(struct node * startNode){
    ...
    //change previous' next node to the one following the last part of the operation
...
int verifyInput(struct node * head){
    ...
    if((temp->type == operator_type || temp->type == parenthesis_type) && (temp->operator == ')' || isCalcOperator(temp))){
        return 0;
    }

    //verify number of parentheses in list by first counting number of opening Parentheses
    ...
    while(temp->next != NULL){
        if(temp->type == parenthesis_type && temp->operator == '('){
            //checks if the node following an opening parenthesis is another opening parenthesis, then moves the pointer until it points to non-parenthesis
            while(temp->next->type == parenthesis_type && temp->next->operator == '('){
                    temp = temp->next;
            }
            //return 0 if next type is not a number, as the loop ensures it can't be an opening parenthesis and any other option is invalid
            ...
            //if the node following the number node is neither a calc operator nor a closing operator, return 0, numbers and closing parenthesis are allowed to be in last position
            if(temp->next != NULL && ( !(isCalcOperator(temp->next)) && !(temp->next->type == parenthesis_type && temp->next->operator == ')'))){
                return 0;
            }
            temp = temp->next;
            continue;
            ...
            //if the node following an Operator is not a Number or a closing Parenthesis, return 0
            if(temp->next->type != number_type && (temp->next->type == parenthesis_type && temp->next->operator == ')' )){
                return 0;
            }
        ...
        //as closing and opening parentheses behave differently in context, they need their own verification
        else if(temp->type == parenthesis_type && temp->operator == ')'){
            //if the node following a closing parenthesis is neither NULL (ending node) nor an operator, return 0;
            if(temp->next != NULL && !(isCalcOperator(temp->next)) && (temp->next->type == parenthesis_type && temp->next->operator != ')')){
                return 0;
            }
...
void printNodes(struct node * head){
    ...
    while(temp!= NULL){
        ...
        switch(temp->type){
            case(number_type): printf("%d", temp->number); break;
            case(parenthesis_type): printf("%c",temp->operator); break;
            case(operator_type): printf("%c", temp->operator); break;
        }
        temp = temp->next;
    }
}
