...
int main() {
    ...
    double punkte = 0;
    double maxpunkte = 0;
    int ATcounter = 0;  //um Doppelzaehlung bei maxpunkte von AT zu verhindern für den Fall, dass erste AT-Punkteeingabe 0 ist.
    ...
    else if (prozent(ZT + AT + UE, maxpunkte) >= 67 && (prozent(ZT + AT + UE, maxpunkte) < 78)) {
        endergebnis(ZT, AT, UE, maxpunkte);
        printf("\nNote: 3");
    }

    else if (prozent(ZT + AT + UE, maxpunkte) >= 78 && (prozent(ZT + AT + UE, maxpunkte) < 89)) {
        endergebnis(ZT, AT, UE, maxpunkte);
        printf("\nNote: 2");
    }
